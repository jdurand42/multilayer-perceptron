...
class ANN:
    ...
    def BackPropagation(self, x, z, Y, w, b, phi):
        ...
        self.delta.insert(0,  (z[len(z)-1] - Y) * phi[len(z)-1](z[len(z)-1], der=True))
        
        '''Now we BACKpropagate'''
        # We thus compute from next-to-last to first
        for i in range(0, len(z)-1):
            self.delta.insert(0,  np.dot( self.delta[0], w[len(z)- 1 - i] ) * phi[len(z)- 2 - i](z[len(z)- 2 - i], der=True) )
        ...
        for i in range(0, len(self.delta)):
            self.delta[i] = self.delta[i] / self.X.shape[0]
